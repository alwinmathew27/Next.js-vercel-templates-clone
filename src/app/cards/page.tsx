...
export default function CardDetails({ searchQuery, activeFilters }) {
  const filteredCards = carddata.filter(card => {
    ...
    const filterMatch = Object.entries(activeFilters).every(([category, items]) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {filteredCards.map((card) => (
          <Link key={card.id} href={`/cards/${card.id}`}>
            <div className="bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer h-full">
              ...
              <div className="p-4 flex flex-col">
                ...
                <div className="mt-4 pt-2 border-t border-gray-100">
                  <span className="text-sm text-gray-500">{card.author}</span>
                </div>
              </div>
            </div>
          </Link>
